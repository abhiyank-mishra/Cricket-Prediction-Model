...
def feature_engineer_match_winner(df):
    print("\n--- 1. XGBoost Match Winner Prediction ---")
    ...
    # Map classes to contiguous integers 0...N
    ...
    # XGBoost Classifier
    clf = XGBClassifier(n_estimators=300, max_depth=6, learning_rate=0.05, random_state=42, eval_metric='mlogloss')
    clf.fit(X_train, y_train_mapped)
    
    preds = clf.predict(X_test)
    acc = accuracy_score(y_test_mapped, preds)
    print(f"XGBoost Match Winner Accuracy: {acc*100:.2f}% (Time taken: {time.time()-start_time:.2f}s)")

def feature_engineer_player_runs(df):
    print("\n--- 2. XGBoost Player Runs Prediction ---")
    start_time = time.time()
    ...
    player_match['career_runs_avg'] = player_match['career_runs_avg'].fillna(0)
    ...
    preds = np.maximum(0, preds)  # Runs can't be negative
    mae = mean_absolute_error(y_test, preds)
    print(f"XGBoost Player Runs Predictor MAE: {mae:.2f} runs (Time taken: {time.time()-start_time:.2f}s)")

def feature_engineer_quick_out(df):
    print("\n--- 3. XGBoost Quick Out Prediction ---")
    start_time = time.time()
    ...
    preds = clf.predict(X_test)
    print(f"XGBoost Quick Out Accuracy: {accuracy_score(y_test, preds)*100:.2f}% (Time taken: {time.time()-start_time:.2f}s)")
...
if __name__ == "__main__":
    ...
    print("\n[Done executing XGBoost pipelines]")
